##
# Wraps an _output parameter_ from the output fields of DIVAServices. Similar to a field.
class OutputParameter < ActiveRecord::Base
  ...
  before_update :adapt_fields, if: :output_type_changed? # Need to make sure that old fields for a previous output_type are removed and exchanged.

  has_many :fields, as: :fieldable, dependent: :destroy
  belongs_to :algorithm

  validates :output_type, presence: { :unless => (output_type = "")}, if: :review_or_step_2?
  ...
  ##
  # Removes all fields related to this output parameter and then creates the new ones.
  def adapt_fields
  ...
  def set_position
    self.position = self.algorithm.next_output_parameter_position
  end

  def create_fields
    data = DivaServicesApi::Algorithm.output_information
    data = data[self.output_type]
  ...
  def deep_copy
    output_parameter_copy = self.dup
    self.fields.each{ |field| output_parameter_copy.fields << field.deep_copy }
    output_parameter_copy.save!
    output_parameter_copy
  end
